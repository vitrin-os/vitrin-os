...
- **These sites hold the literal mask** — the list is the gated one, so it is stated without a count in front of it — and a new bit turns each red until a human re-pins it with the reason: `crates/vitrin-protocol/tests/decode_errors.rs` (the fatal/recoverable boundary) and `crates/vitrin-core/src/attention.rs` (D-023's "no attention verb is allocated" pin). The `verb-valid-mask` MIRROR row in `crates/xtask/src/limits.rs` renders the *generated* constant into both, and into this paragraph and `docs/protocol/04-vitrin_grant.md`, so a literal left behind anywhere is a red build rather than a stale sentence. Its sibling test scans the tree for literal pins and fails if one exists that this list does not name — which is the half that had gone wrong.
