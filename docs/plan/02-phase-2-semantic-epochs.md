...
**Verb bits** (`Verb::VALID_MASK` is **703** today — `1|2|4|8|16|32|128|512`, the six original bits plus `realm_launch` and `egress`; it read 63 until WS-E.1.1 landed the 512 row below, and 575 until P2.7.2 landed the 128 row). Re-pin the mask **once per epic**, never once per task, in its **four** sites: `crates/vitrin-protocol/tests/decode_errors.rs`, `crates/vitrin-core/src/consent/render.rs`'s unserved-set catalogue test, `sdk/python/tests/test_verb_parity.py`, and — **added to this list by P2.7.2, which found it the expensive way** — `crates/vitrin-core/src/attention.rs`'s `the_two_exempt_verbs_are_the_two_layout_verbs_and_no_bit_is_allocated`, which pins the mask literally to prove that D-023 allocated no bit and therefore fires on *every* verb addition, including ones that have nothing to do with the attention key. A list that named three sites while four existed is the same defect class as a stale enumeration anywhere else in this repo: it is discovered by a surprise red, and a reader meeting that one is invited to misread it as "this change allocated an attention verb".
...
| 128 | `egress` (**bit landed**; unserved — no facet, no proxy) | E2.7 | P2.7.2 (#196) |
...
| `13-vitrin_powerbox.md` | Powerbox facet (`designate_file`) | P2.6.5 |
...
| `19-vitrin_egress.md` | Egress facet (`egress`) — **a second interface, not a request on page 13**, because `interface/@verb` is one value per interface; allocated by P2.7.2 (#196), which landed the verb bit without it | P2.7.3 |
