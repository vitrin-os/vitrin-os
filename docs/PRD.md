...
- **Human preemption.** Physical input carries top priority; when a physical event arrives, in-flight agent actuations **to the same focus** are preempted and the agent's actuator is transiently suspended (the libei "suspend EI during a password prompt" idea, generalized). "The same focus" is per realm and not per session: physical input is addressed to the realm the human's attention is bound to, an agent's actuation to the realm its grant names, so a human working in one realm does not suspend an agent working in another. Hold-Esc is the exception and is deliberately session-wide — it revokes the active principal's grants over every realm, because an off-switch scoped to what the human happens to be looking at is not an off-switch.
...
- **Clipboard broker.** No shared clipboard. A copy from realm A places data into a core-held buffer only on an explicit, user-initiated action; a paste into realm B is a second explicit action. This is the Qubes Ctrl-Shift-C/V model — "fully controlled by the user, it cannot be triggered/forced by any" realm — generalized to principals and grants. **Partly built as of WS-E.2.1 (issue #213, [D-024](plan/20-decision-log.md)):** one core-held slot, `text/plain;charset=utf-8` only at 60 KiB, promoted by Ctrl-Shift-Insert and offered by Shift-Insert — Ctrl-Shift-C/V is not expressible, because a core-owned chord may only use keys whose keysym no keymap moves (letters are not among them, on any backend -- see [D-028](plan/20-decision-log.md)), and Insert is. What is *not* built is the "generalized to principals and grants" half: there is no agent-facing clipboard verb, and that is E3.5's. The hardened broker, drag-and-drop and the portal layer are E3.5's too.
...
- **Escape consequence, corrected by measurement (P2.6.4, 2026-08-16).** The reachable-service escape class **as enumerated above** (`ssh localhost`, `flatpak-spawn --host`, mounted container sockets, D-Bus activation, `cron`/`at`) is closed by construction, not by blocklist: there is simply nothing to reach (§15 threat row). Designated egress is the only outbound path, and it is host:port-scoped and audited. **But "there is simply nothing to reach" was written as a claim about the class and is only true of the enumeration**, and the difference was measured rather than argued: from inside the full realm ladder, a realm could search the operator's kernel keyring by name and read a key's payload in plaintext (the keyring is namespaced by nothing a realm unshares, and possession is inherited across `unshare(2)`), and could open an `AF_VSOCK` socket — whose addressing the network namespace does not scope, so on a VM guest it is a path to the hypervisor that own-netns does not close. Both are now closed by blocklist and not by construction: they are rows of the P2.6.4 seccomp deny-list (`vitrind --print-seccomp`). The general sentence to carry forward is the narrower one — *every service this section enumerates is closed by construction; two that it did not enumerate were reachable, and a deny-list is what closes them.*
...
| ------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Malicious app in a shim**                                                                                                          | Corrupt/crash its own shim; render hostile pixels in its own surface; exhaust its own realm's resources | Observe/address any other realm, surface, or principal; read another app's buffer or tree; synthesize input elsewhere; reach the session's real seat or a11y bus; *initiate* any cross-realm transfer (its universe is only itself). **Narrowed for the clipboard as of WS-E.2.1 / D-024** and stated as a bound rather than an absence: a human at the physical keyboard can now move `text/plain;charset=utf-8`, at most 60 KiB, one direction per gesture, between two realms. A realm cannot trigger, force or observe that — the core asks, and there is no message by which a shim announces a copy — but two colluding realms can move 60 KiB per human gesture pair, which is the Qubes bound and is published in `docs/book/src/limits.md`                                                                                                                                   |
...
| **Compromised shim**                                                                                                                 | Lie about its own app's buffers/tree; misbehave toward its own app                                      | Escape its sandbox into the core (unprivileged, and **seccomp-filtered against a named deny-list since P2.6.4 — which is not the same as seccomp-confined**: `vitrind --print-seccomp` prints the rows that are closed and the residual syscall surface is unenumerated; isolation dial per §4.5); affect other shims/realms; forge a realm identity (assigned by the core at fork, not claimed by the shim); tamper with the journal (append-only, signed by the core)                                                                        |
...
15. **Per-principal cursors, and who may see whose:** §5.1's P1 promises each principal "its own cursor (or cursorless)" — thirty agents and a human means thirty-one cursors, not one contended pointer. Today's implementation does not do this and says so: `consent/grab.rs` and `input/mod.rs` both record that "v0 shares one cursor between origins", and `protocol/vitrin-v0.xml` contains **no cursor concept at all**, so the wire cannot currently express the model the PRD claims. Three questions follow. (a) Does the shared cursor survive v0, given that `ConsentGrab` needs a defensive rule — never let emulated motion move the position the human's hit test reads — that exists *only* because one cursor is shared, and that per-principal cursors would delete rather than complicate? (b) Cursor **visibility is a relation, not a flag**: a human supervising thirty agents needs per-agent toggles to keep the screen readable; agent-to-agent visibility leaks what another principal is doing; and an agent reading the *human's* cursor learns the human's attention and can time its actions to it — which is closer to `observe` than to a display preference, and so arguably belongs in the grant model. (c) Must cursors be **core-rendered**, like the trusted band and the consent card? A realm that draws its own cursor bitmap can paint a decoy and mislead the human about where input is going — the same spoofing class as the consent-replica problem (§5.2, issue #85). The vocabulary question is the urgent one: cursor identity, visibility, and the cursorless case are protocol surface, and adding them after v0 freezes is a version bump against deployed clients. **Decided 2026-07-26 — see decision log [D-017](plan/20-decision-log.md).** The model is settled and on the wire: the pointer facet *is* the cursor's name (no new object), cursorless is by construction (never petitioning `actuate_pointer`), a captured frame contains no cursor except the human's — and that one only for a grant holding the new verb `observe_cursor`, which is meaningful only alongside `observe`, while another *agent's* cursor is unpurchasable at any verb set — and cursors are core-composited so a realm can never paint a decoy. The visibility rule is deliberately asymmetric; what D-017 refuses is a per-pair toggle *matrix*, not a per-grant verb. **What remains open and is deferred to M2:** per-principal cursor *delivery* — v1 still ships one shared pointer position per realm view, so `consent/grab.rs`'s defensive rule survives through M1, and `observe_cursor` is refused `unsupported` rather than served. **Amended 2026-07-26 — see [D-019](plan/20-decision-log.md).** Cursor *drawing* is no longer deferred and "composites no cursor at all" is no longer true: the core composites each actuating agent's own cursor into human-visible output only — never a captured frame — from an agent-owned position (`input/mod.rs`'s `InputRouter::agent_pointer`) distinct from the shared one it delivers. D-019 supersedes only that half of D-017's deferral; per-principal *delivery* and `observe_cursor` stay deferred to M2, and the agent-to-agent exclusion stays unpurchasable at any verb set.
16. **Is layout a capability?** Read the ordinary window-manager verbs as authority rather than decoration: `raise` puts a surface *over* the consent card; `move` slides a different target under the pointer mid-decision; `focus` redirects keystrokes meant for another realm; `fullscreen` impersonates the whole session; `resize` pushes the prompt out of the human's visual centre. Each is the same class of attack `consent/grab.rs` already defends against on the pointer axis — an agent that could "slide the hit target under the human's finger and turn a click aimed at Deny into an Allow". §5.2's consent surface is unspoofable today partly because **nothing outside the core can do layout at all**: there is no window manager yet. That is a property of Phase 1's narrowness, not a design, and Phase 3's `mission-control shell v0 (unprivileged)` ends it — the moment a client arranges realms, something outside the TCB performs layout. "The shell is trusted" is not available as an answer, because §5.1 makes it an *invariant* that window-management policy lives outside the core (the Nitpicker/Scenic split), so trusting the shell would move exactly the code that invariant exiles back into the TCB. Q15's per-principal cursors close the *aiming* half of this — you cannot slide a target under a pointer you do not share — but layout manipulates the **shared scene**, so the exposure survives. The shape worth evaluating: layout verbs become attenuable, revocable, journaled verbs on a grant (*"this shell may arrange realm-7 and realm-9, may not steal focus, may never fullscreen"*), while the core enforces **ordering invariants no grant can purchase** — nothing renders above the consent surface or the trusted band — the same way it refuses actuation under `ConsentHeld`. Two costs to weigh honestly: arbitration between two principals holding layout grants risks dragging window-management *policy* into the core, which the invariant forbids (a single layout-holder per output may be the escape); and layout changes invalidate an agent's cached coordinates, which is the staleness problem the epoch/CAS mechanism (§6.2) already exists to solve, so the two should be designed together rather than in sequence. **Decided 2026-07-26 — see decision log [D-018](plan/20-decision-log.md).** Layout is grant-governed (verbs `layout_arrange` and `layout_focus`, focus separately attenuable because focus theft is both the sharpest attack and the most legitimate need); the core enforces ordering invariants no grant can purchase (nothing composites above the consent surface or the trust indicator; the core's own hit test, never a client's claimed stacking, routes input); arbitration is refused in favour of one layout-holder per output, because arbitration is the window-management policy §5.1 exiles; and E2.3's epoch MUST be defined over frame content *plus* view geometry *plus* stacking, with layout mutations bumping it. **Enforcement landed early, for the nested single-output case, at WS-E.1.4 (issue #210) — see [D-022](plan/20-decision-log.md).** Both verbs are served through two facets (`vitrin_layout_focus`, `vitrin_layout_arrange`) defining exactly two requests, `focus` and `set_fullscreen`; `place`, `resize`, `raise` and stacking exist as **no request at all**, because a scene showing one unstacked realm cannot honour them. A second `layout_arrange` petition resolves the appended outcome `layout_held`. D-018(2)'s invariants are now **tested as invariants**: a client holding every served verb drives the whole expressible arrangement space through the real wire and fails to touch the consent card, to move where input lands, or to put its cursor in any capture — and a fifth invariant is added, that the human's own physical input reaches only the realm the output is bound to. **What remains deferred:** the epoch D-018(5) binds layout to (E2.3) does not exist, so there is a real window in which an agent's cached coordinates can be invalidated by a shell with no compare-and-swap available to notice; multi-output arrangement; and everything the scene cannot express.
