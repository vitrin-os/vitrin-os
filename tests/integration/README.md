...
| `test_hostile_client.py` | **Component test.** P1.9.3 (#46)'s *policy* half, which `fuzz/` structurally cannot make: given a real Unix socket and a real forked realm, a peer sending bytes no conformant SDK would construct is killed while the core and every other connection keep serving. Two disjoint outcomes are asserted, matching where each violation is caught — **protocol-level** ones (`pre_handshake`, `auth_failed`, `version_unsupported`, a second `hello`, a truncated `hello` payload) get a best-effort `vitrin_handshake.error` before the close, while **transport-level** ones (a header lying about its own minimum size, an fd count the protocol cannot express, an fd flood) are caught in `vitrin-ipc` below the layer that could attach an error to anything, so the peer gets a bare close and that absence is itself the thing under test. Plan §5's P1.9.3 row asks for "hostile-client tests in CI" and this is them, but it is **not** one of D12's named mock-free gates: the far seam here is `vitrin-mock-shim`, and the memory-safety half of #46 is `fuzz/`'s. | No |
...
| `test_real_ransomware.py` | The **M2.5 ransomware gate** (P2.6.9 ★, #193; M2.5 closes with ★P2.7.6 #200): the repo-authored `ransom-payload` runs in a real realm and its **measured write set equals exactly {designated fds} ∪ {realm private storage}** — a set equality, not an "implemented confinement" claim. Real `vitrind` → real `vitrin-realm-init` → real `vitrin-shim` → real `ransom-payload` → real SDK, no mock on any seam; the payload holds the per-realm `designation.sock`, so this is the **app-side mock-free receipt P2.6.6/P2.6.7 left owed** (the shim's disposition is `relayed`, not `no app is connected`). **The write set outside the grant is empty, confined:** the payload attempts a self-cleaning `O_EXCL` canary create into the host home, `/`, `/etc`, `/run` and the core.sock's directory (all refused `ENOENT`/`EACCES`) and into its own private storage `{/run/vitrin,/vitrin/home,/tmp,/dev/shm}` (all succeed, being the realm's own) — the equality's two halves. **The positive control is in the same run:** at `--isolation=off` (no mount namespace) the host home the confined run could not reach IS reachable, so the negative is not satisfied by a home that was never reachable (the #138 lesson); the payload only ever creates and unlinks one `O_EXCL` canary, never a real file. **The designated fds are the only writable authority, and the journal is a second witness:** the agent designates a read and a read-write fixture, the payload writes through each (read-write succeeds, read-only is the kernel's `EBADF`), and the core's `designation_settled` journal — its `(st_dev, st_ino)` set and its delivered count — equals what the payload reported receiving, so a payload that lied would contradict the journal. **A symlink row is refused, not followed** (`RESOLVE_NO_SYMLINKS` → `unresolvable`), and the "racer wins without it" control is authored in-process here (the guarded half is `picker/resolve.rs`'s in-crate racer, the confirm→open window being unsynchronisable from a test process). **The kernel and measured tier are printed** (`vitrind --print-isolation`), and the write-set claim is qualified where the P2.6.3 ladder reports a missing right (below Landlock ABI 3 a read-granted *path* is still truncatable — Correction 2). **Two disclosures.** (1) `ransom-payload` is **repo-authored** (`shim/tests/ransom_payload.c`, R2.7): no third party enumerates its own write attempts; mitigated by the third-party rungs (`test_real_app.py`, `test_real_gtk.py`, `test_real_firefox.py`) staying green in the same run, by the journal as an independent witness of the designated writes, and by a receipt-frozen breakage in the watched-failing list — and it does **not** rule out a payload bug under-reporting its own attempts against *undesignated* paths (the journal never sees those), which is why each such target is shown reachable at `--isolation=off`. (2) The **picker-spoofing rung** (`--replica-picker` + the trust-band witness) needs a backend that stacks the consent grab; headless does not, so it runs on **DRM on an isolated VT** (`VITRIN_RANSOM_DRM=1`, a hardware run) and **SKIPS** otherwise — until it and ★P2.7.6 land, M2.5 is not closed. `DemoUsesNoMockShim`-style ancestry (`vitrin-mock-shim`/`mock_core.c` appear nowhere in the process tree, asserted by inode) rides along. Same C-shim env contract as the rest of the real-app ladder. | **Yes — M2.5 (ransomware half)** |
...
| `test_real_powerbox.py` | The **designation property gate** (P2.6.6, issue #190) — mock-free, real-app, deliberately **not** a milestone gate: M2.5's named gate is ★P2.6.9 (`test_real_ransomware.py`), the milestone row above; this file is #190's property gate, and no plan document schedules a milestone here. A real agent's SDK `request_file` against the shipped `vitrind` raises the real core-drawn picker over a real `click-target` under a real C shim; the picker is driven through the consent channel's four picker verbs (`list`/`navigate`/`confirm`/`cancel`, which land in the same `ConsentGrab` queue a keystroke does and are applied by `session::service_picker_round`), and the descriptor that arrives over `SCM_RIGHTS` is **the inode of the row the picker displayed** — `(st_dev, st_ino)` compared against a pair recorded when the file was created, **never against a path re-resolved at assertion time**, because re-resolving a name after the fact is precisely the race the picker exists to close. It is then **spent, not held**: `pread` returns the designated file's bytes, and a `request_file(write=True)` designation is `pwrite`n through and read back — the arm that makes `mode` falsifiable at all, since a core ignoring the ask would answer `read` to everything and every other assertion here would still pass. Two refusal arms deliver **nothing**, asserted by counting this process's own `/proc/self/fd` across the ask: a cancelled picker answers `refused(cancelled)` *specifically*, and a **symlink row** answers `refused(unresolvable)` — `picker::resolve` walks with `RESOLVE_NO_SYMLINKS|BENEATH|NO_MAGICLINKS`, so this is the resolver's defining flag exercised over a real socket rather than in-crate. **The picker reaches no capture**, and the claim is made in the order #138 taught: the exported occlusion window is first shown to **be** a picker raster at exactly the rectangle the core named — accent ring on all four edges, no accent anywhere but that ring and one slot stroke, both horizontal edges of that stroke complete, opaque body, buttons, antialiased text, and **the selected-row highlight on the slot the `list` reply independently reported, moving by exactly `PANEL_ROW_H` when the cursor moves by one row** — and only then is the absence read: the core-internal `--capture-dump` realm view and the agent's own `observe()` frame are **byte-identical** to settled controls taken before the card went up, agree with each other through the M1.3 gate's own comparator, and carry **zero** px of the card's accent and selected-row colours, both of which are counted in the export at that same instant so the two zeroes are a measurement rather than a tautology. The journal carries one `designation_settled` per admitted ask — `(st_dev, st_ino)` on the two that delivered, **neither field** on the two that did not, plus mode, grant row, principal and realm — joined to the SDK's own grant through the chokepoint's `use_decision.grant_wire_id`. A second class is the gate's positive control: the same core, grant and ask with **no `[[picker]]` table** in `realm.toml` are refused `internal` before a card exists, which is what makes that one line of config load-bearing rather than decorative (there is deliberately no `$HOME` fallback). A third is binary-free: the provenance checker is fed an empty buffer, a flat card-shaped forgery and a card whose highlight is the wrong shape, and must reject all three, against a fourth that it accepts a correct one. **Not asserted:** that a human at a real keyboard drives it (no input device on a runner — the injector's verbs bypass `judge` exactly as #138's `decide` does); anything about `request_dir`; the `openat2` floor (a claim about kernels below 5.6, owned by `picker::resolve::probe`); or the plan cell's **path racer**, whose window is sub-millisecond and is therefore sequenced in-crate in `picker/resolve.rs` rather than raced from a test process. **One seam ends early and is disclosed rather than implied:** the core sends the one descriptor twice, and the realm's half is decoded by the real C shim — this gate greps the shim's own `realm.log` for that decode, id, kind, mode and name length, the only witness available on the far side of that wire — and then **disposed of as the relay decides**: the per-realm `designation.sock` exists as of P2.6.7 (#191), but the realm's app in this gate is `click-target`, which never connects to it, so the disposition the shim logs for every designation here is `CLOSED, not relayed: no app is connected` and the app never receives it — by the app's choice, not the shim's. The app-side mock-free proof (a powerbox-aware app under a real core receiving over `designation.sock`) is ★P2.6.9's; the component proof is `shim/tests/acceptance/designation_relay.sh` below. | No — property gate (#190) |
...
| `shim/tests/acceptance/designation_relay.sh` | **Component test, and registered in `shim/meson.build` (run by `meson test -C shim/build`), not by `run.sh`** — it is listed here because this table is the repo's answer to "what did this run actually prove" about the designation path, and the shim's half would otherwise be invisible from it. The shim-side designation relay (P2.6.7, #191): a real `vitrin-shim` spawns the MPL-2.0 test instrument `shim/tests/designation_client.c` as its app — so the client runs with the app's real composed environment and fd table, and asserts fd 3 is closed before it opens anything — against `shim/tests/mock_core.c` sending N `vitrin_shim_session.designation` frames. Arms: **(A)** `$XDG_RUNTIME_DIR/designation.sock` exists, is a socket, mode `0700`, in the runtime dir, and is **announced by nothing** — the shim's `/proc/<pid>/cmdline` and `environ` contain no `designation.sock`, and the ledger's `designation-sock:` record says `tier=host-path` (a script runs on the host path; `tier=in-realm` is what the core's `/run/vitrin` produces); **(B)** the shim-spawned client receives the designation and its decoded id/kind/mode/name equal the mock core's `EV designation` line, `(st_dev, st_ino)` equal the mock core's, `pread` returns the subject's bytes, `object_id=1`, and the ledger's `designation-relay … outcome=relayed` and the shim log's frozen prefix + `relayed to the connected app` both appear; **(C)** a read-only designation refuses `write` with `EBADF` **and** the positive control — a `--designate-mode read_write` run writes and reads back — without which `EBADF` would be meaningless; **(D)** 1000 designations at 1 ms: the shim's `/proc/<pid>/fd` census — taken while the client is alive and **polled** back to baseline, because the client's `RX` line can precede the transport's close by microseconds — returns to baseline with zero links resolving to any designated subject, and the client's own fd count is flat; **(E)** a client that writes on the socket is dropped (`wrote_and_closed bytes=N`), and a designation sent afterwards is `outcome=no_client` with the shim log saying `no app is connected`; **(E2)** the same with an `SCM_RIGHTS` descriptor attached to the write — no `/proc/<shim>/fd` link names the attached file, because the drain is `read(2)` and the kernel drops what it cannot install; **(F)** a second connection while one is held sees EOF and the ledger says `refused_occupied`, with the first connection still receiving; **(G)** a `--stall` client that never reads, under 1000 designations (`G_COUNT`): the ledger shows `send_failed errno=EAGAIN` once the shim socket's send buffer fills (278 messages on a default-tuned kernel — the bound is `net.core.wmem_default`, not `net.unix.max_dgram_qlen`, which the kernel skips for a connected peer), then `no_client`, and the census settles at baseline minus the dropped connection's two descriptors (the connection and its event-loop dup) with zero links to any subject — the arm that catches a relay that closes on success and forgets on failure; **(H)** `designation-summary:` totals equal the per-record counts; **(I)** nine rapid refusals print exactly eight records and `suppressed_refused=1`. Vacuity guards on every census: the held connection's `socket:[ino]` link is present in both, the mock core's `EV` count equals N, the client's `RX n=N` line exists. **Not asserted:** anything about a real core (the frames come from `mock_core.c`), or that a human's designation reaches an app — that is ★P2.6.9's. | No — component test (mock_core.c) |
...
2. **`comm` stopped identifying the shim.** It is bound at a core-chosen
...
One thing here **was** a workaround for a defect rather than a fact of nature,
and it is gone because the defect is (#283). A Meson build-tree shim linked
against the vendored wlroots carried the RUNPATH `$ORIGIN/subprojects/wlroots`;
`$ORIGIN` inside the realm is the bind target's directory, so the dynamic
loader killed the shim before `main` and the core reported only `Broken pipe`.
CI always hit it — Ubuntu ships wlroots 0.17.1, so `shim/ci/install-deps.sh`
installs wlroots' *build* dependencies and the vendored subproject is always
used. The harness worked around it by computing the shim's non-system library
directories with `ldd`, binding them read-only and forwarding
`LD_LIBRARY_PATH`, which made the width of the realm's mount table a function
of a build flag.

`shim/meson.build` now links anything vendored **into** the shim
(`default_library=static`, D-043), so the harness adds no binds of its own and
seeds no `LD_LIBRARY_PATH`. `meson test loader-independence` holds that at the
build, and the real-app gates below hold it end to end: if the RUNPATH returns,
they fail with `Broken pipe` rather than passing over a harness that quietly
widened the realm.
...
| the one socket the walk found (2026-08-23; two since P2.6.7, see below) | the shim's own `/run/vitrin/wayland-0` | the shim's own `/run/vitrin/wayland-0` | — |
...
The two confined arms' `1 socket` cells are also a reading of their date: P2.6.7
(#191) later gave the shim a second socket, `designation.sock`, beside
`wayland-0`, and the table has **not been re-run** since — the instrument's
allowlist and vacuity guard (below) are what hold the new count, not this table.
...
the realm's whole filesystem, was truncated by nothing, and found only the
shim's own AF_UNIX inodes: on the date of the table above, one (`wayland-0`);
since P2.6.7 (#191), exactly the shim's two sockets (`wayland-0` and
`designation.sock`, the relay's, bound before the app is forked), which the
instrument now allow-lists by derivation from `$WAYLAND_DISPLAY` and
**requires** to have been found and answered as a live `SOCK_SEQPACKET` socket
(`EPROTOTYPE` to the walker's `SOCK_STREAM` probe, the kernel's answer only
after it has located a live socket by inode), so an allowlist entry for a
socket that is not there cannot pass. Its three `P311-WALK-DENIED` rows are `ENOENT`,
...
found no AF_UNIX inode other than the shim's own (one on that date; exactly the
shim's two sockets since P2.6.7). The `off` arm's walk hit its 60000-inode budget
