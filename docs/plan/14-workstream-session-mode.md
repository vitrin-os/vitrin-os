...
**CLOSED 2026-08-13** — epic #206 and all nineteen task issues, #207–#224 and
#232. **Realigned 2026-08-25 by [D-047](20-decision-log.md)**, which gave this
workstream the two things its closure exposed it never had: a numbered
**E-series exported-artifact contract** (**§9** — until it existed, the plan
tree's own rule made every WS-E artifact an internal detail no later phase was
entitled to rely on, and Phase 3 already reaches for several) and a **Stage 5**
(**§4.5**) owning the four decisions it produced that are accepted, unbuilt and
scheduled nowhere. The three hardware rungs that were written and never run get
their own lane in **§4.6** and no date. None of it re-decides anything: the
residue is sequenced, not reopened.
...
**unchanged by this workstream** (575 at the time, 767 today — P2.6.5 appended
`designate_file` and P2.7.2 appended `egress`) — because nothing agent-facing is
added; no new prose page, since these
...
## 4.5 Stage 5: the four decisions this workstream took and did not build

**Added 2026-08-25 by [D-047](20-decision-log.md)(1), which is the owner's.**
WS-E closed on 2026-08-13 — epic #206 and all nineteen task issues, #207–#224
and #232 — and four accepted decisions came out of it that are **decided,
unbuilt, and scheduled nowhere**, plus ten open issues that no row of the stage
table above holds. Stage 5 is where they live.

**Stage 5 sequences them and re-decides no clause of any of them.** D-038's
decisions 1–2 and D-046's decisions 1–3 are the owner's and stand exactly as
written, unbuilt included;
[D-025](20-decision-log.md#d-025--a-locked-screen-does-not-suspend-agent-observation-the-gap-is-published-not-papered-over)
is untouched, and whoever builds the shell realm re-asserts its ordering as a
test rather than inheriting it as an assumption. Nothing below is a new
decision, and a later issue that finds one of them surprising is reading the
decision log, not this section.

**Two shapes were refused at D-047(1) and are recorded so they are not
re-proposed.** *Folding this residue into Phase 2 as a new epic* — it would put
session-mode work inside a phase whose exit criteria are semantic, and
[02-phase-2-semantic-epochs.md](02-phase-2-semantic-epochs.md) is already the
least-tracked document in the tree. *A successor workstream, WS-F* — WS-E's
closure is a fact worth keeping rather than blurring, and a fifth workstream
document is a maintenance cost with no offsetting clarity when a stage inside
this one carries the same rows.

### The four decisions

| Decision | What it decided | What is unbuilt | Issue |
|---|---|---|---|
| **[D-038](20-decision-log.md)** (2026-08-17) | The desktop shell becomes **a realm that draws**: one realm holding both a shim surface and a principal connection, so it draws like any other app while holding `layout_focus`, `layout_arrange` and `realm_launch` through the ordinary grant path. The core stops drawing the status strip and composites that realm into the rows below the trust line. Decisions 1–2 are the owner's. | The shell realm itself. **This row is smaller than it was on 2026-08-17**: the `ViewGeometry` prerequisite D-038 named as *"the single largest piece of unbuilt work this decision implies"* has since landed (§9, E8), so what remains is the shell, not the inset. The core cannot verify the pairing of the drawing half and the holding half, and D-038 states that as the security content of the decision rather than as a gap to close later. | [#304](https://github.com/vitrin-os/vitrin-os/issues/304) |
| **[D-039](20-decision-log.md)** (2026-08-17) | A global hotkey is served as a **named action** the core resolves, never as a keystroke it forwards; D-023's one attention bit becomes an operator-configured chord table; and **no verb for observing the human's input is designed**, deliberately rather than by omission. Decisions 1–2 are the owner's. | All of it, and **the gating deliverable is a paired IDL + prose edit that nothing schedules.** `vitrin_principal.attention` cannot carry it — *"NO ARGUMENTS, AND THAT IS PERMANENT"*, because signatures are immutable — so the action arrives as a new `since`-gated sibling message plus its `docs/protocol/` page, validated with `xmllint --relaxng protocol/vitrin-v0.rng protocol/vitrin-v0.xml`. That is why the issue is `track:protocol` even though the resolver is core code. A chord this build or backend cannot resolve must be refused at startup, on `deadman::Chord::parse`'s fail-closed precedent. | [#305](https://github.com/vitrin-os/vitrin-os/issues/305) |
| **[D-040](20-decision-log.md)** (2026-08-17) | Layer-shell and tiling are **one deferral rather than two**, because both need a scene that holds more than one client surface. X11 stays E3.2's; multi-output stays behind the M4 gate. | The scene. `crates/vitrin-core/src/scene/mod.rs:253` is still `surface: Option<SurfaceContent>` — a single `Option`, not a collection. **Not a protocol gap:** D-022(4) records that the arrangement requests are absent *because the scene cannot honour them*, and `vitrin_layout_arrange`'s own description says the absences are the design. This is why §2's bars-and-launchers row (waybar, rofi, wofi) still fails: there is no `zwlr_layer_shell_v1` for them to map into. | [#307](https://github.com/vitrin-os/vitrin-os/issues/307) |
| **[D-046](20-decision-log.md)** (2026-08-24) | A shell realm reaches the core socket through **a descriptor the core mints** and passes down the `spawn.rs` socketpair channel it already has — no filesystem object, no named socket, no Landlock rule, and the realm's mount table stays closed. The authority is a `realm.toml` declaration **and** a human's consent at the `while_running` rung, both and not either. What the connection may carry is fenced **structurally**: the layout verbs and `realm_launch` yes; `observe`, `actuate_pointer` and `actuate_text` refused at the chokepoint regardless of consent. Decisions 1–3 are the owner's. | The mechanism. **And it has no open issue at all**: [#311](https://github.com/vitrin-os/vitrin-os/issues/311) asked the question and is closed by the answer, so what landed from it is the *measurement* — `tests/integration/p311_principal_socket_reach.py`, 14 computed routes to `core.sock`, 0 reached in both confined arms against 6 in the `--isolation=off` control — and not the descriptor. Anyone reading the tracker for this work will find nothing. | *(none)* |

### The residue, ten issues

Every one of these was produced by WS-E and outlived it. They are listed here
because a closed epic cannot hold them and no other document does.

| Issue | What it is | Note |
|---|---|---|
| [#234](https://github.com/vitrin-os/vitrin-os/issues/234) | A launched realm has no reclamation path: revocation, disconnect and the dead-man switch all leave it running | WS-E.1.1's residue. E3.3 inherits it — a fleet that can launch and not reclaim is a leak with a cap on it |
| [#252](https://github.com/vitrin-os/vitrin-os/issues/252) | The capture cache recomposes every live realm on every dirty round, whether or not anything will read it | Partly answered by **[D-035](20-decision-log.md)** (skip a realm whose scene has not changed); **gating on consumers stays refused**, because a capture's freshness must not depend on whether a grant happened to exist. Open on the remainder |
| [#253](https://github.com/vitrin-os/vitrin-os/issues/253) | The shipped shim only ever attaches `kind=shm`, so no real app can reach the core's zero-copy dmabuf path | **D-047(6)** records M1.5's [#171](https://github.com/vitrin-os/vitrin-os/issues/171) as **code-blocked by this, not hardware-blocked** — the shim sets `.kind = SHM` as a literal at its single attach site, so the zero-copy path is unreachable by construction. Not an off-machine item, and it must stop being framed as one |
| [#275](https://github.com/vitrin-os/vitrin-os/issues/275) | A gesture interrupted by a VT switch is delivered `completed`, not `cancelled`: libinput flushes before the core's drain | The one failure of step 13a's six rungs, found on the target laptop 2026-08-13. A shipped defect in an artifact §9 exports (E5) |
| [#277](https://github.com/vitrin-os/vitrin-os/issues/277) | Route 2's `kill -TERM` cannot recover the wedge `L6` itself produces: a stopped process queues SIGTERM and does nothing | A published recovery command that cannot rescue the case the runbook creates. §7's escape route is the surface it touches |
| [#303](https://github.com/vitrin-os/vitrin-os/issues/303) | Backlight actuation (**[D-041](20-decision-log.md)**) | **Landed; off-machine.** Counted in §4.6's lane and not against Stage 5's engineering |
| [#304](https://github.com/vitrin-os/vitrin-os/issues/304) | The shell realm (**D-038**) | Its `ViewGeometry` half landed; the shell did not |
| [#305](https://github.com/vitrin-os/vitrin-os/issues/305) | Hotkeys as named actions (**D-039**) | The paired IDL + prose edit above |
| [#306](https://github.com/vitrin-os/vitrin-os/issues/306) | Idle inhibition (**[D-042](20-decision-log.md)**) | **Landed; off-machine.** Counted in §4.6's lane |
| [#307](https://github.com/vitrin-os/vitrin-os/issues/307) | A scene holds one client surface (**D-040**) | The blocker under layer-shell and tiling alike |

### What Stage 5 does not do

It gives four decisions and ten issues a **home**, and nothing more. **It sets
no order inside itself, carries no estimate, and carries no date.** D-021's cost
bullet records WS-E's budget as roughly Phase 2's remaining one and calls the
priority the maintainer's to make; a stage added after the workstream closed
does not get to spend it for him. It closes nothing, builds nothing, and moves
no published limit — every sentence `docs/book/src/limits.md` carries about the
absences above is still true on the day this section was written.

## 4.6 The off-machine lane: three rungs no amount of engineering clears

**Added 2026-08-25 by [D-047](20-decision-log.md)(5), which is DERIVED and not
the owner's**, and marked as such on the D-044 / D-041 / D-042 precedent a
reader of this tree will look for.

Three rungs of [`docs/drm-bringup.md`](../drm-bringup.md) are written and have
**never been run**. Each has its deliverable fully landed — code, tests, docs,
merged — and each is blocked by exactly one thing no amount of engineering
clears: **a bare-metal run on the one machine §5 describes.** CI cannot reach
any of them, and not for want of trying: no runner has a seat, a VT, a display
controller, an ACPI table or a `/sys/class/backlight` at all. Under this repo's
mock-free definition-of-done these runs are not optional extras — they are the
evidence the claim cannot be made without, which is why counting them as
ordinary open work makes every estimate wrong in the same direction.

**They are therefore removed from every completion count in this document, and
no date is set here.** The session is the maintainer's to book, and a date
invented in a plan document is exactly the commitment
[00-roadmap.md](00-roadmap.md) refuses outright.

| Rung | Owns | What the run must produce | Status |
|---|---|---|---|
| **§12a** (`12a-i`, `12a-ii`) | The `immediate` and `idle` answers of `--lock-on-seat-change` (**[D-034](20-decision-log.md)**, issue #246, which is **closed**). Step 12 exercised only the default, `never`, so two shipped policies have never been executed by anyone | A fresh `vitrind` per policy, each with a `--lock-passphrase-file` — without one the card is a privacy screen and Enter dismisses it, which proves the raise but not the cost. **`immediate`:** a VT round-trip returns to the lock card and the card names the **seat**, not an idle timer that never fired; the journal's `session_locked` cause reads `seat`. **`idle`:** a ~10 s absence returns **unlocked** with the absence charged against the countdown; a >60 s absence returns **locked**, cause `idle`, on the first round after returning; and a >60 s absence followed by typing immediately stays unlocked for a further 60 s, because physical input is the one thing `judge` clears the carry for | **NOT YET RUN.** Closes no issue — #246 is closed already. It is the only evidence two shipped policies could ever have |
| **§16** (`16-i`…`16-viii`) | Backlight actuation (**[D-041](20-decision-log.md)**), issue [#303](https://github.com/vitrin-os/vitrin-os/issues/303) | The device and its `max_brightness`, read from a **second VT** between presses. A press that moves the panel *and* the file by one step — `max_brightness × 5%` rounded **up**, never below one raw unit, so a small-`max` device moves by 1 unit and that is the floor working rather than failing. **The panel itself moving, not only the number.** A floor that never reaches 0 — the one **safety** defect on the rung, because a black panel is indistinguishable from a blanked one. The app in the realm not seeing the keys. Exactly one `backlight_stepped` entry per press. Nothing at all without `--backlight`, with the app receiving the key. And the external-display row, which is correct, published, and the thing a human will find confusing | **NOT YET RUN.** **The literal before/after values and the device's `max_brightness` pasted onto #303 are the issue's acceptance criterion, and nothing else can stand in for it** |
| **§17** (`17-i`…`17-vii`) | Idle inhibition (**[D-042](20-decision-log.md)**), issue [#306](https://github.com/vitrin-os/vitrin-os/issues/306) | At minimum `17-ii`, `17-iii` and `17-v`, in words, naming the app used. The shim's relay and the core's per-realm edge both logged — **check the app actually asked before concluding anything about the core**, because an app that never asks makes every row vacuous. The panel staying lit well past a 60 s `--blank-idle` under a full-screen video with hands off the keyboard. The panel blanking again within ~60 s of the video stopping — a leaked inhibit is the one failure this feature can cause that a human cannot work around except by killing something. A `kill -9` of the app leaving the blank on schedule. And, with `--lock-idle 120`, **the lock coming up at 120 s while the video plays and the panel stays lit**: an inhibit that suppressed the lock would be a confined app switching off a security control, which is D-033(1)'s exact prohibition and D-042's central bound | **NOT YET RUN.** A green `meson test` cannot stand in for it: `shim/tests/acceptance/idle_inhibit.sh` runs against `shim/tests/mock_core.c` and never touches a panel |

**What this lane is not.** It is not a claim that these features are broken.
Each is landed with tests: the code behind #303 and #306 merged with its unit
and component coverage, and #246's two unrun policies shipped with theirs. It is
a claim about **evidence**: the honest status of all three is *landed in the
tree, unproven on hardware*, which is what
[`docs/book/src/limits.md`](../book/src/limits.md) already publishes for the
idle inhibit and what §4.4's deferral block already says of the backlight.
Nothing here changes a published sentence; it names why those sentences cannot
yet change.

**Nor is it the whole of D-047(5)'s lane.** That decision counts **six** issues
repo-wide held only by an off-machine criterion. The table above is WS-E's
contribution to it and does not enumerate the rest.

**Two named-but-unrun items are deliberately outside this lane, because neither
needs the maintainer's hardware.** The **VKMS rung** — `vkms-advisory` in
`.github/workflows/ci.yml`, run against a `--blank-idle` session — is a CI job
anybody can execute, and §"What is built, what is not, and what cannot be known
here" already carries it as unclaimed. And **#253** is code, not hardware: see
§4.5's residue table, and D-047(6), which is the entry that stopped it being
filed behind a needs-a-GPU excuse.
...
- <!-- limit: lifecycle-checklist-run-once -->
  **Suspend and lid have been exercised once, on 2026-08-11, short of the counts
  ...
  > **AMENDED 2026-08-13.** The entry above is left as written because it is the
  > honest status *of the first execution*, but its counts were superseded by the
  > runs of 2026-08-12 and 2026-08-13. `L2` and `L3` now stand at **5 of 5** —
  > the fifth suspend cycle, and three more lid cycles that all reached sleep, on
  > 2026-08-13 — `L6`'s unreconstructable route was recovered (`kill -CONT`
  > against a 163.8 s `SIGSTOP` wedge), `L7` has the measurement it asked for
  > (**61.214 s lit, counted from the seat's return**, against a 60 s timeout),
  > and #258's and #259's fixes were read on 2026-08-12. The published register
  > this entry is paired with — `lifecycle-checklist-run-once` in
  > [`docs/book/src/limits.md`](../book/src/limits.md) — already reads that way:
  ...
  > a clean pass."* **Neither run being a clean pass is the half that did not
  > change**, and the id this bullet is gated on is unchanged, so the pairing
  > holds. The dated records are on
  > [the recovery page](../book/src/recovery.md#the-numbers-this-checklist-owes).
...
## 9. Exported-artifact contract — the E-series (what later work may depend on)

**Added retroactively on 2026-08-25 by [D-047](20-decision-log.md)(1), twelve
days after this workstream closed.** Phase 1 exports A1–A6
([01-phase-1-mvp.md](01-phase-1-mvp.md) §1) and Phase 2 exports C1–C8
([02-phase-2-semantic-epochs.md](02-phase-2-semantic-epochs.md) §1) under one
rule, stated in both: *anything not listed here is an internal detail free to
change.* **This document had no such section at all.** Under that rule every
artifact below was, until this section existed, something no later phase was
entitled to rely on — and Phase 3 already reaches for several. So this is a
contract written after the fact about work that is already landed, and it says
so rather than pretending to have been written first.

Each entry carries the same four things C1–C8 carry: **what the artifact is**,
**the WS-E task and issue that produced it**, **who may consume it**, and **the
limitations it travels with**. That last one is the whole point of the
mechanism — an exported capability whose caveat is left behind in the issue that
built it is an artifact a later phase will over-read. Two limitations below must
never travel without their artifact: **a locked screen does not suspend agent
observation** (E7), and **the backlight does nothing at all for an external
display** (E6).

**Why this section is numbered 9 rather than 1.** This file's section numbers
are cited from outside it — `docs/drm-bringup.md` cites §5 and §7,
[00-roadmap.md](00-roadmap.md) cites §4's stage table,
[20-decision-log.md](20-decision-log.md) cites §4.1 and §6, and
`docs/book/src/session-app-matrix.md` cites §2 — so inserting a section at the
front would silently move every one of those. Appending is the same reflex this
page already applies to its own claims.

- **E1 — The bare-metal DRM/KMS backend.** `crates/vitrin-core/src/backend/drm.rs`, reached by `--drm` and built only behind the **non-default `drm-backend` feature**: a build without the feature has no `--drm` arm in `parse_args` at all and exits with ``unknown argument `--drm` ``. DRM/KMS mode setting, a GBM swapchain scanned out by the panel, GLES, libinput for the human's real devices, libseat for the seat. It composites through the same `human_visible_from_view` and `dmabuf::present_human_visible` the nested backend does, so the trusted band reaches it from the existing draw lists rather than from a third one, and it serves captures from `Scene::compose` and **never** from the scanout buffer. It is the one backend that composites the **human's** cursor — [D-029](20-decision-log.md)(2), which had to correct an IDL sentence stating flatly that no human cursor is composited at all — and the one that implements `Ctrl-Alt-F<n>` itself (the second **D-031**, after refusing to is what trapped the human on `tty3`). Produced by WS-E.3.2/#218, WS-E.3.3/#219 and WS-E.3.4/#220. **Consumable by:** nothing in Phase 2 or Phase 3 depends on it, and **D-021(2) forbids citing any of it as evidence toward M4** (§0). What it exports is that a third presentation backend exists behind the same `Presenter` contract as `--nested` and `--headless`, driving the same `scene`/`session` runtime. **Limitations:** it drives **exactly one output** and refuses to start on zero or more than one connected connector, D-029(1), for a contract reason rather than a content one — six named places in the runtime each have one answer and would need two. It enumerates **once**, installs no udev monitor, so a connector hot-plugged mid-session is neither lit nor complained about and unplugging the only panel leaves the session compositing where nobody can see it. `--drm` requires `--consent=interactive` and refuses `auto-approve`, D-029(3). **No hardware cursor plane** — a full composite per pointer move — and the human's own pointer is invisible in the top `TRUST_BAND_HEIGHT` rows, because the sprite inherits the band's clip. On a real seat **a same-uid app can open `/dev/input/event*` directly** and bypass this core entirely, consent grab included; nesting hid that, it did not prevent it. It cost the largest single TCB dependency increase in the project's history — 14 crates and four system libraries, five of them `-sys` crates doing unsafe FFI. And **no green check in this repository will ever show it setting a mode, presenting a frame or delivering a key**: CI holds a compile check and device-free unit tests, and the mock-free evidence is a human executing [`docs/drm-bringup.md`](../drm-bringup.md) — of which three rungs have never been run (§4.6).

- **E2 — Multi-realm sessions.** `MAX_REALMS = 16` (`crates/vitrin-core/src/realm.rs:628`), raised from one by WS-E.1.2/#208; a session runs **every** realm its `realm.toml` declares, each with its own shim, runtime tree, socket, **`Scene` and capture** (WS-E.1.3/#209), with exactly one scene bound to the output and `realm-0` mandatory. A realm declared `autostart = false` is a **template**, instantiated under a core-minted `<template>.<n>` id. Launch capacity counts **live** realms and forgets exited ones (`RealmRegistry::capacity_used`), and a launch at the cap is refused `capacity` — `Refusal::Capacity` at the single enforcement chokepoint (`crates/vitrin-core/src/enforcement.rs:584`, `:1164`), journaled as `capacity` (`recorder.rs:2172`). **Consumed by** E3.3, which generalizes this into a fleet, and E3.2. **Limitations:** every realm renders whether or not it is visible, so `MAX_REALMS` has a **measured memory bill** and not only a descriptor one — published in `docs/book/src/limits.md`, re-derived in `realm.rs`, and open as **#252**. **A launched realm has no reclamation path**: revocation, disconnect and the dead-man switch all leave it running — **#234**, which E3.3 inherits rather than discovers. **One view size for the whole session**: `Presenter::view_size` is read once before the first realm forks and configures every shim with it (D-029(1)), so a second output is not merely unimplemented but inexpressible. And **the spawn chain a realm is started through is not WS-E's and is not exported here**: since **D-037** (P2.6.2, #186) the core `exec`s `vitrin-realm-init`, which forks a supervisor and unshares in the PID-1 child before `execve`ing the shim — two trusted binaries, whose confinement properties belong to Phase 2's **C4**. WS-E exports the registry, the cap, the per-realm resources and the refusal. **It exports no confinement property whatsoever.**

- **E3 — The three served session verbs, and their three facets.** `realm_launch` through `vitrin_grant.get_launcher` → `vitrin_launcher` (WS-E.1.1/#207, protocol half #225); `layout_focus` through `get_layout_focus` → `vitrin_layout_focus`; `layout_arrange` through `get_layout_arrange` → `vitrin_layout_arrange` (WS-E.1.4/#210, **[D-022](20-decision-log.md)**). All `since="2"`, all structural mints, each facet born inert and refusing `not_granted` forever if its grant never holds the verb. **One interface per verb is structural, not stylistic:** `interface/@verb` is single-valued, so focus and arrangement cannot share a facet and stay independently attenuable — a shell granted arrangement but not focus holds a live `vitrin_layout_arrange` beside a `vitrin_layout_focus` that can never work. `vitrin_launcher.launch` **carries no arguments**: the grant names a template, the template names the program, so no command line ever crosses the wire and the human sees that program on the consent card. `vitrin_layout_arrange` is one request, `set_fullscreen`, and *the absences are the design* — there is no `place`, `resize`, `raise`, `lower`, `close` or stacking request, **not a request that refuses but no request at all**. **Consumed by** E3.3, E3.8, and today by `examples/shell/run_shell.py` (E9). **Limitations:** a launch grant is authority over operator-written configuration and **bounds nothing about what the launched app may then do** — that is Phase-2 confinement (E2.6/E2.7), and `limits.md` says so. `layout_arrange` is **single-holder per output** (`layout_held` answers the second arranger), a session-wide singleton E3.3 must not assume generalizes to a fleet. Growth arrives as `since`-gated sibling requests *when the scene can honour them*, and the scene cannot: see §4.5, **#307**. And these verbs are reachable only by a principal that has a socket — which, until **D-046** is built, a realm does not have (§4.5).

- **E4 — The cross-realm clipboard.** A **core-held single slot** (`crates/vitrin-core/src/clipboard.rs`), filled only when the human presses **Ctrl-Shift-Insert** in the realm the output is bound to and offered only when they press **Shift-Insert** in another; `text/plain;charset=utf-8` only; **61 440 bytes**, measured rather than chosen for roundness; cleared on timeout, on the source realm's death, and on a dead-man trigger. The core **pulls** — `request_selection` → `selection` → `offer_selection` — and *"the core pulls"* is a type rather than a rule: `PendingPromotion` is not `Clone`, has no public constructor and is consumed by value, so an unsolicited `selection` from a compromised shim has nothing to consume and fills nothing. WS-E.2.1/#213, **[D-024](20-decision-log.md)**; five axes and every rejected option are §4.1. **Consumed by** E3.5, which owns the agent-facing verb and the fleet-scale claim. **Limitations:** **no agent-facing verb exists**, and the human's path is **not grant-governed at all** — an authority-bearing act with no grant behind it, journaled both halves, on the dead-man switch's precedent, so a reader who expects every channel to appear in the grant table will not find this one. **Two colluding realms can move 60 KiB per human gesture pair**; PRD §15's threat row was narrowed in the same commit rather than left standing. It is **one nested single-output session's clipboard**, so E3.5's fleet-scale criterion looks discharged by it and is not — [03-phase-3-network-x11-fleet.md](03-phase-3-network-x11-fleet.md) already records that trap. The cap lives in the IDL's immutable `(max N bytes)` token, so raising it is a new message rather than an edit. And **the Shift-Insert chord is spent, on the historical X11 primary-paste chord**: the core eats both chords in every realm unconditionally with no pass-through, so an X client in a realm loses its own paste gesture *before* an X path exists. WS-E.4.1/#221 handed that to **E3.2** as the third of six requirements (§4.2), and E3.2 inherits it as a collision it did not choose.

- **E5 — The seat vocabulary additions.** On `vitrin_shim_seat`: `relative_motion`, and four gesture events — `gesture_begin`, `gesture_swipe_update`, `gesture_pinch_update`, `gesture_end`, over `gesture_kind` (swipe, pinch) and `gesture_state` (completed, cancelled). On `vitrin_shim_session`: the `pointer_constraint` ask with its `pointer_constraint_state` verdict, and `idle_inhibit` with its state enum. On `vitrin_principal`: the **`attention`** event. All `since="2"`. Produced by WS-E.4.2/#222 (**[D-032](20-decision-log.md)**), WS-E.1.7/#232 (**[D-023](20-decision-log.md)**), and #306 (**[D-042](20-decision-log.md)**), with the shim-side globals in `shim/src/`. **Consumed by** E3.2, which must relay the same vocabulary for X clients, and E3.7. **Limitations:** **none of these is a grant, a verb bit or a facet, and that is structural rather than pending.** `pointer_constraint` and `idle_inhibit` are asked for by the **confined app** over its shim connection, and there is no principal to key a grant on — `Verb::VALID_MASK` was unchanged by D-042, and `constraint.rs` pins the absence with a named test so it stays deliberate. E3.7 must not read either as attenuable authority. An **idle inhibit is honoured only while the core's output is bound to the realm holding it**, and it suppresses the **blank** and never the **lock**. `attention` has **no arguments and that is permanent** — signatures are immutable — and it lifts `preempted` for one use of the two layout verbs and delegates nothing. **Touch and tablet are deferred** against named reopening evidence (D-032), so "the seat vocabulary" must not be read as complete. **#275 is open against a shipped defect in this artifact**: a gesture interrupted by a VT switch is delivered `completed` where it must say `cancelled`, found on the target laptop by rung 13a-v on 2026-08-13. And the idle inhibit is **landed in the tree and unproven on hardware** — rung 17 is written and NOT YET RUN (§4.6), and `shim/tests/acceptance/idle_inhibit.sh` is a component test against `shim/tests/mock_core.c`.

- **E6 — Backlight actuation.** `crates/vitrin-core/src/backlight.rs`: on `--drm --backlight` the core consumes `XF86MonBrightnessUp`/`Down` and **writes** `/sys/class/backlight/<device>/brightness` — the one filesystem write the trusted core makes on its own behalf. 5% of `max_brightness` per press with a floor at 5%, both rounded **up** and never below one raw unit, so the published floor is literally 5% and not `floor(max/20)`. Bounded exactly the way `status/battery.rs`'s read is: one fixed `&'static Path` root that nothing on the wire can influence, an operator-nameable **leaf only** (`--backlight-device`, a `DeviceName` that cannot be constructed with a separator in it), a cap on how many directory entries are examined with names collected-sorted-**then**-truncated in that order, and a cap on every read enforced by the reader's own type. **No panic and no error path**: every failure is the key doing nothing, journaled as `backlight_stepped`. **[D-041](20-decision-log.md)**, issue **#303**. **Limitations, both of which D-041 accepted rather than answered:** it is a **second** display-power interface inside the TCB and the blank state machine knows nothing about it — D-033's refusal of the backlight as a *blanking* mechanism stands entirely untouched, and nothing here is reached by a timer. And **it does nothing at all for an external display**; rung 16-viii records that as correct, published, and the thing a human will find confusing. Without `--backlight` — and on `--nested`/`--headless`, where the flag is a startup error — both keys are still delivered to an app that cannot act on them. The **volume half** of the same published limit did not move. And the honest status is **landed in the tree, unproven on hardware**: no runner has a seat, an ACPI table or a `/sys/class/backlight` at all, so the evidence is rung 16, written and NOT YET RUN (§4.6).

- **E7 — The lock screen, the idle blank, and the seat-change policy.** A core-drawn lock screen on the consent stack (WS-E.2.2/#214, **[D-025](20-decision-log.md)**, `crates/vitrin-core/src/lock/`); `--blank-idle SECS`, a `--drm`-only idle blank whose DPMS is `DrmSurface::clear()` and whose activity clock is written at **exactly one site** — inside `LockScreen::judge`, below its `Origin::Physical` check — so a blank cannot silently postpone the idle lock and an agent's actuation neither postpones nor wakes it (WS-E.4.3/#223, **[D-033](20-decision-log.md)**); `--lock-on-seat-change immediate|idle|never` (#246, **[D-034](20-decision-log.md)**); and what the trusted band asserts across a VT switch and a dark panel (WS-E.3.3/#219, **[D-030](20-decision-log.md)**). **Consumed by** E3.7 and by anything that reasons about the human's presence. **Limitations — the first must never travel without the artifact:** **a locked screen does not suspend agent observation.** A principal holding `observe` keeps capturing across the lock, frame for frame, and one holding `actuate_pointer`/`actuate_text` keeps acting. That is **D-025**, the maintainer's own call of 2026-08-08, taken with the alternatives in front of him in exactly those words, and it is **published — on the card, in `docs/book/src/limits.md`, and in §6 — not papered over**; being surprising is the reason it is published, and it is not to be re-litigated by a later issue that finds it so. **Idle blanks and does not lock** (D-033(1)): an unlocked session can sit behind a dark screen, and because a disabled CRTC produces no vblank, **a dark screen is not evidence that anything is being observed live either**. Suspend is detected **after the fact** from the monotonic/wall clock pair, or not at all; lid and power are logind's, and this repository does not own those settings. `--blank-idle` is refused at startup on `--nested` and `--headless`. And the hardware evidence is a human executing `L1`–`L7` on [`docs/book/src/recovery.md`](../book/src/recovery.md): **all seven rungs have been run**, across 2026-08-11, 2026-08-12 and 2026-08-13, and that page's [*The numbers this checklist owes*](../book/src/recovery.md#the-numbers-this-checklist-owes) carries `L2` and `L3` at **5 of 5** as of 2026-08-13 — the 4-of-5 and 2-of-5 counts elsewhere on this page are the first run alone and are scoped to it, and the second run is where liveness was first proven, the first having passed no `--keymap` and so having no way to tell an idle app from a frozen one. What is still unexecuted sits off that ladder: **routes 3 and 4 are unexecuted predictions** — "Route 3 is documented and unexecuted" and "route 4 has never been used", both to be treated as careful predictions — and rung 12a's two other seat policies have **never been run** (§4.6).

- **E8 — The permanently inset realm view.** `crates/vitrin-core/src/view.rs`'s `ViewGeometry`: one value carrying the output's size **and** the rows the core draws its own chrome into, threaded through the five consumers that place client pixels or tell the app its size — `Scene::compose`, `Scene::take_damage_view`, the input router's `surface_local`, `dmabuf::human_visible_frame`, and `ShimServer::send_configure`. Landed as a required task of **#304**. The trusted band's 8 rows are reserved on **every** presented frame with no arm of either compositor path omitting it; the status strip's rows are reserved only under `--status`, and `ViewGeometry::new` takes the strip's **height** rather than a flag because `0` is the honest spelling of "no strip", not a special case. **The exported property is the one a later phase is most likely to assume the opposite of: there is no such thing as a session whose view is not inset.** **Consumed by** whatever builds D-038's shell realm, and by any later path that places client pixels — the carrier exists precisely because three such paths previously received the view size from three different places and shared no carrier for a second number. **Limitations:** the reservation is a **derivation chain**, not a set of constants — `STRIP_TOP` from `TRUST_BAND_HEIGHT`, `dmabuf::status_strip_rect` from `STRIP_TOP`, `ViewGeometry::reserved_top` from that — so a consumer that restates the band's height has left the chain, and the band's height appears in `view.rs` nowhere. It is **one carrier for one output**, and D-029(1) is why there is never a second. And it makes D-038's client-drawn status rows **possible without building them**: the inset landed, the shell realm did not (§4.5).

- **E9 — The switcher and launcher as an ordinary SDK client.** `examples/shell/run_shell.py`, WS-E.1.5/#211, closed 2026-08-08. It holds `layout.focus` per realm, `layout.arrange` over exactly one output and `realm.launch` per template, all through the normal petition-and-consent path, and turns `list` / `focus` / `launch` / `fullscreen` into the four wire requests those verbs authorise **and nothing else**. It never refuses on the core's behalf — a denied grant still sends, and shows the core's `not_granted` — and never retries a refusal. Driven as a subprocess by `tests/integration/test_shell.py`. **This is the one place PRD §5.1's *"window-management policy lives outside the core"* is paid for rather than asserted**, which is what makes it an exported artifact and not a demo. **Consumed by** E3.8 as its starting point, and by anyone testing E3's verbs end to end. **Limitations:** it is **not a desktop shell and structurally cannot become one here.** It is **line-oriented** because `vitrin_view` is capture-only and no principal-facing surface interface exists — no client can put a pixel on the output — and it is **hotkey-free** because there is no `observe_input` verb. Its own `README.md` argues both as authority questions rather than ergonomics, and `docs/book/src/limits.md` publishes them. Both absences are Stage-5 work and neither is scheduled: drawing is **D-038** (#304), hotkeys are **D-039** (#305) — and D-039 keeps `observe_input` **undesigned on purpose**, because the hotkey arrives as a named action the core resolved and never as a keystroke a client can read.
