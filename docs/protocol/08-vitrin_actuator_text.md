...
- *Recoverable* refusals arrive as [`vitrin_grant.refused`](04-vitrin_grant.md#refused)`(verb = actuate_text, code, retry_after_ms)`. The applicable [`refusal`](04-vitrin_grant.md#refusal) codes are the full **actuation** set — eight of the enum's nine: `not_granted` (grant pending, denied, dead, or `actuate_text` outside the effective verb set), `expired`, `revoked`, `rate_limited` (with `retry_after_ms > 0`), `preempted` (physical human input owns the target), `consent_held` (a consent prompt is up; agent actuation is refused, never delivered to the app), `no_surface` (the realm's shim crashed or exited), and `internal`. The ninth, `capacity`, is launch-only (version 2) and can never refuse a `type`; see [`vitrin_launcher`](16-vitrin_launcher.md).
