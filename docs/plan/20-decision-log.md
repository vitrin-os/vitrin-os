...
1. **The position is agent-owned, not the shared one.** `input/mod.rs`'s `InputRouter::agent_pointer` is a second field beside `InputRouter::pointer`, written **only** when `origin == Emulated` and cleared by `reset()`. The shared field is written by both origins — that is exactly the delivery D-017 deferred — so a sprite drawn on it would follow the human's physical mouse and tell the human an agent is pointing wherever the human is. This is the mirror image of `consent/grab.rs`'s `ConsentGrab::pointer`, which is deliberately *physical*-only so an agent cannot slide a hit target under the human's finger: same idiom, one origin over.
