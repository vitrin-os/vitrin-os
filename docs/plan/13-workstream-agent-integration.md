...
| `capacity` (8) | `AtCapacity` | The deployment is full and cannot start another realm. Reachable only through `realm_launch`, which the reference core serves since WS-E.1.1. Not a rate limit: `retry_after_ms` is 0 and the core cannot know when a realm exits, so a retry timer will spin. |
