...
| `backend/` (`winit.rs`, `headless.rs`) | Doc 2 §9 (rendering/compositing path); PRD plan E3 (P1.3.1–2) | The two presentation backends: `--nested` (a host Wayland client, real display) and `--headless --size WxH` (fixed virtual output, pixman software rendering, no GPU — the CI path, plan decision D3). Both drive the same `scene`/`session` runtime. |
...
| `dmabuf.rs` | Doc 2 §3.4, §9 (zero-copy dmabuf import) | The `DmabufImporter` trait seam and the shm-fallback decision (D3) it exists alongside. **Not yet threaded into either backend's runtime path** (`importer: None` in `session.rs`/`lifecycle.rs`) — real-GPU zero-copy is open issue [#117](https://github.com/vitrin-os/vitrin-os/issues/117); see the root README's gaps list. |
...
| `consent/` (`mod.rs`, `render.rs`, `canvas.rs`, `text.rs`, `grab.rs`, `indicator.rs`) | Doc 2 §5.3 (consent surface); plan D4 (TCB rendering stack) | The core-rendered consent prompt — composited above the realm view, in human-visible output only, drawn with a hand-rolled canvas + `fontdue` (no GUI toolkit in the TCB) — plus its exclusive input grab while a prompt is shown, and the "this session is under vitrind's consent surface" trust indicator. |
