...
| [`vitrin_powerbox`](docs/protocol/13-vitrin_powerbox.md) | Designation facet (since wire version 2) — ask the human to pick one file or one directory subtree and have the **descriptor** delivered to the realm; no path crosses the wire in either direction. **Served by `vitrind` since P2.6.6 landed the core-drawn picker** (issue #190); a deployment with no picker root it can open answers `internal` rather than serving a verb with nothing behind it, and the consent copy naming what approving it costs landed at P2.6.8 (issue #192, D-048) |
