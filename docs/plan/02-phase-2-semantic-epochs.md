...
> **REALIGNED 2026-08-25 BY D-047.** The row above stands as written and is the
> standing record of what P2.1.2 was scoped to do. Two of its clauses are spent, and
> issue [#324](https://github.com/vitrin-os/vitrin-os/issues/324) already carries them
> in this corrected form — the table is behind the tracker, not the other way round.

**Correction 1 — the 1→2 bump is not P2.1.2's to make.** The Task cell ends *"and bump
`protocol/vitrin-v0.xml`'s root `@version` from 1 to 2"*. That bump landed **2026-08-06
in WS-E.1.1** (`6abe8dd`) and version 2 has been appended to many times since; the root
element reads `version="2"` today. What P2.1.2 still exclusively owns is the
**argument-reservation discipline** §3's second serialization point describes: a
signature is immutable forever ([00-conventions.md](../protocol/00-conventions.md) §7.4),
so `flags` on `tree_ready`, `since_version` **and** a reserved `options` word on
`observe_tree`, and the `observed_epoch` shape E2.4 needs, must all exist at this
landing or that epic buys a sibling request forever.

**Correction 2 — the version-matrix criterion is struck, and moved rather than dropped.**
The Acceptance cell requires *"a v1 client handshakes and completes a capture
unchanged"*. **D-047 decision 4 (the owner's, 2026-08-25) declares protocol version 1
dead**: the shipped core accepts exactly its maximum and refuses every other integer,
version 1 included, so this criterion was red on `main` before the rung it belongs to
had started. It is legitimate to strike because **D-032**(6) records that version 2
*"has never been released, frozen or negotiated by any implementation outside this
repository"* — there is no client anywhere that declaring version 1 dead would break.

**The debt is recorded, not discharged.** The per-connection version matrix becomes an
obligation of **WS-A's spec 1.0-candidate rung**
([10-workstream-spec.md](10-workstream-spec.md)), because the moment a third party
implements the published protocol, "an older client keeps working" stops being
theoretical and a core that speaks only its newest version is a defect rather than a
disclosed gap. D-047 refuses building it inside P2.1.2 by name: it would sit on the
critical path of the phase the realignment is restarting, for a compatibility that
currently has no consumer.

**A fourth site, named because D-047's own enumeration missed it.** D-047 decision 4
strikes the dual-version clause from *three* sites — C1's export text, §3's walking
skeleton, and §4's M2.1 rung. This task row is a fourth, and it was found by the
verification pass on #324 rather than by the realignment. Recorded here so the count in
D-047 reads as the enumeration it was, not as a claim of completeness.
...
| ID | Task | Key decisions | Depends on | Acceptance criteria | Track |
|---|---|---|---|---|---|
| **P2.4.1** | Extend E2.2's tree vocabulary in `protocol/vitrin-v0.xml` plus its prose page with the synthetic-publish path: a `vitrin_tree_publisher` facet minted by `vitrin_realm.request_grant` under a new `vitrin_grant.verb` bit `publish_tree` (**0x100**, from §5's registry — this row said `0x40` until P2.6.5 landed `designate_file` on that bit, which is exactly the collision the registry exists to prevent) carrying `publish_tree(surface, tree, observed_epoch)`; a tree-level `synthetic` flag | Q3's v0 posture becomes wire vocabulary, not sidecar convention — a convention cannot bind a lying publisher. Confidence is a per-node byte rather than a float: it must survive the codec unambiguously and be comparable across backends. `accept_synthetic` is a request flag, NOT a verb | P2.1.1, P2.1.2 | `xmllint --relaxng protocol/vitrin-v0.rng protocol/vitrin-v0.xml` green and `cargo xtask codegen --check` idempotent; the verb tripwires §5 names go red until a human classifies the new bit — both literal `Verb::VALID_MASK` pins and `crates/vitrin-core/src/consent/render.rs`'s unserved-set catalogue test (§5 carries the site list; repeating it here is how it went stale last time) | `protocol` |
...
| **P2.6.6** | Build the core-drawn file picker and the fd-minting path: navigate/select/confirm chrome on the existing consent stack, race-free resolution with openat2 RESOLVE_NO_SYMLINKS from a directory fd, delivery over SCM_RIGHTS, subtree designation as a directory fd, and one journal record per designation. | Picker shape: core-drawn IN-CORE, reusing the consent renderer (tiny-skia + cosmic-text per D4 — **CORRECTED 2026-09-10 while building P2.6.6 (#190); the parenthesis is wrong twice over and is left standing rather than rewritten, per this document's correction convention.** D4 (`docs/plan/01-phase-1-mvp.md` §5) says *"tiny-skia + fontdue"*, never cosmic-text; and the shipped renderer uses **neither** tiny-skia nor cosmic-text. `crates/vitrin-core/src/paint/canvas.rs` is a hand-rolled opaque RGBA canvas and `paint/text.rs` rasterizes through `fontdue` alone, with `fontdue -> ttf-parser` the entire subtree. `crates/vitrin-core/Cargo.toml` records both as **declined** against plan risk R7 — tiny-skia adds 7 crates for the shapes fontdue does not draw, and cosmic-text is *"the usual 'just do text properly'"*. P2.6.6 therefore extended `paint/` rather than importing a rejected dependency into the TCB, and `cargo tree` shows neither. The picker also needed a **second glyph source** the cell does not anticipate: the vendored face carries no kana and no Han, so Japanese filenames are drawn from a pre-rasterized atlas (`paint/atlas.rs`, 3152 glyphs at one size, no shaping tables in the file at all) rather than from a subsetted CJK font), not a core-owned separate process — a separate process needs its own trusted-path story from scratch, while the consent surface already has ConsentGrab's exclusive input grab, the trusted indicator and the trust band, so picker spoofing reduces to the… | P2.6.5, P2.6.2, P2.6.3, A3 | A path-race test: a repo-authored racer swaps a component of the chosen path between the human's confirm and the open, and the delivered fd's fstat (st_dev, st_ino) must equal the entry the picker displayed; where openat2 is unavailable the picker REFUSES to designate rather than falling back to a racy resolve, asserted. | `rust-core` |
...
#### P2.7.2, the conformance clause discharged

**DISCHARGED 2026-08-27 BY [#322](https://github.com/vitrin-os/vitrin-os/issues/322),
IN HALF, AND THE OTHER HALF IS NAMED RATHER THAN ROUNDED UP.** The row above stands
as written and is the acceptance record as it was accepted; this subsection restates
its final criterion rather than replacing it, on the same footing as "P2.6.3,
corrected" and "P2.1.2, corrected" above.

**Past tense as of 2026-08-27: *"the reference core dispatches none of the four new
opcodes — `vitrind` answers `get_egress` and `request_connect` `invalid_opcode`"*.**
It did, and that is exactly the defect [#322](https://github.com/vitrin-os/vitrin-os/issues/322)
was filed for: a defined request of the negotiated version killed a conformant
client's connection. It no longer does. `vitrin_grant.get_egress` (opcode 4) mints
the structural facet the IDL describes — always legal, born inert, no wire
acknowledgement — and `vitrin_egress.request_connect` reaches the enforcement
chokepoint and refuses **`not_granted`**, recoverably, with the connection intact.
Both are pinned by tests in `crates/vitrin-core/src/principal.rs`:
`get_egress_mints_an_inert_facet_and_never_kills_the_connection`,
`request_connect_refuses_not_granted_and_leaves_the_socket_alive`, and
`every_since_2_mint_on_a_grant_has_a_dispatch_arm`, which derives `vitrin_grant`'s
opcode set from the generated protocol crate so a future mint cannot land
vocabulary-only and reopen this.

**Two of the four opcodes, not four.** `connected` and `connect_failed` are events,
and no code path in the core emits either. Nothing about this task's outstanding work
moved: `egress` is still outside `SERVED_VERB_BITS`, which is *why* `request_connect`
refuses rather than a temporary stand-in for the refusal it will give once the proxy
exists, and **P2.7.3 still owes the proxy, the two delivery events and the served
verb**. What #322 bought is grammar — a conformant version-2 client can mint the facet
and ask, and survive both.
...
**Verb bits** (`Verb::VALID_MASK` is **767** today — `1|2|4|8|16|32|64|128|512`, the six original bits plus `realm_launch`, `designate_file` and `egress`; it read 63 until WS-E.1.1 landed the 512 row below, 575 until P2.6.5 landed the 64 row, and 639 until P2.7.2 landed the 128 row). Re-pin the mask **once per epic**, never once per task. E2.6 and E2.7 each drew one bit from the table below while decomposing in parallel and neither collided, which is the whole of what this registry is for.

**This list was wrong for two epics, and it is now checked rather than remembered.** It used to say "its three sites: `decode_errors.rs`, `consent/render.rs`, `test_verb_parity.py`", and measured against the tree that was wrong in both directions — `crates/vitrin-core/src/attention.rs` holds a literal pin and was not named, while `sdk/python/tests/test_verb_parity.py` holds no literal at all (it re-derives the mask from `protocol/vitrin-v0.xml`). Whoever landed bit 128 by following it would have left `attention.rs` red. So the sites are now stated in the two classes they actually fall into, and `cargo xtask limits-check` gates both the class and the value:

- **These sites hold the literal mask** — the list is the gated one, so it is stated without a count in front of it — and a new bit turns each red until a human re-pins it with the reason: `crates/vitrin-protocol/tests/decode_errors.rs` (the fatal/recoverable boundary) and `crates/vitrin-core/src/attention.rs` (D-023's "no attention verb is allocated" pin). The `verb-valid-mask` MIRROR row in `crates/xtask/src/limits.rs` renders the *generated* constant into both, into this paragraph, into `docs/protocol/04-vitrin_grant.md`, into [20-decision-log.md](20-decision-log.md) and into `protocol/vitrin-v0.xml` — whose normative `<description>` states the mask as a count of defined bits and names the lowest bit it leaves out, so the row renders those two derived forms as well as the decimal. A literal left behind anywhere is a red build rather than a stale sentence. Its sibling test scans the tree for literal pins and fails if one exists that this list does not name — which is the half that had gone wrong.
- **These fire by derivation and hold no literal**, so they need no edit here but still force a human decision: `crates/vitrin-core/src/consent/render.rs`'s unserved-set catalogue pin (the new bit widens the derived unserved set), and `sdk/python/tests/test_verb_parity.py`, which re-derives mask, dotted names and the served/unserved split from the IDL — the edit it forces is in `sdk/python/src/vitrin_os/protocol.py`, not in the test.
- **One site is a classification rather than a pin.** `crates/vitrin-core/src/grants.rs` must place the new bit in `SERVED_VERB_BITS` or deliberately leave it out; nothing fires if it is forgotten, because `UNSERVED_VERB_BITS` is derived and the omission fails *closed* at runtime instead.

**Serving an already-allocated bit allocates nothing and still moves the derived sites.** WS-E.1.4 served `layout_arrange` (16) and `layout_focus` (32) without adding a bit, so `VALID_MASK` did not move and neither literal pin fired — but the served/unserved *split* did move, which turned the catalogue test's unserved-set pin and `test_verb_parity.py`'s served-set constant red. That is the tripwire pair working with no new allocation: a verb cannot become served without a human classifying it and writing its consent-prompt copy.

| Bit | Verb | Epic | Task |
|---|---|---|---|
| 64 | `designate_file` (**bit and facet landed**; unserved — no picker, no consent copy) | E2.6 (earliest-starting) | P2.6.5 (#189) |
...
on the same rule, and P2.7.2 landed **19**, so `docs/protocol/` now ends at
`19-vitrin_egress.md` with the same deliberate gap below it. **Two** pages
...
had anticipated a single "layout facet" would have understated it. **The same
rule split the powerbox the same way**, and P2.7.2 (#196) hit it: `egress`
cannot be a request on an interface that declares `verb="designate_file"`, so
it takes **19** below and page 13 keeps the filesystem half. Two rows have now
been widened from one by the same dialect constraint; a row proposing a single
facet for two verbs is wrong on sight.

Page 19 landed **before** pages 12–15, which is the registry working rather
than a numbering accident: a page number is a permanent address, not a queue
position, so an interface that lands early takes its allocated number and the
gap below it stays reserved.

| Page | Content | Task |
|---|---|---|
| `12-semantic-nodes.md` | Node schema + `vitrin_tree_publisher` folded in as a second interface | P2.1.1 / P2.4.1 |
| `13-vitrin_powerbox.md` | Powerbox facet, `designate_file` (**landed**) | P2.6.5 (#189) |
...
| `19-vitrin_egress.md` | Egress facet (`egress`) — **a second interface, not a request on page 13**, because `interface/@verb` is one value per interface (**landed**; the verb it serves is still refused by every deployment, because the proxy behind the facet is P2.7.3's) | P2.7.2 (#196) |
