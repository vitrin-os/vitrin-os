...
| Q16 | Is layout a capability? `raise`/`move`/`focus`/`fullscreen` are authority over the human's attention, and the consent surface is unspoofable today only because nothing outside the core does layout at all | **mission-control shell (E3)**; protocol spec (E1); epoch/CAS interaction (E2.3) | posture decided **before any unprivileged shell arranges realms (E3)**; core-enforced ordering invariants (nothing above the consent surface) settled with the shell's design |
